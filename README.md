...
set their availability, and add what they need. The household cook gets a live,
aggregated kitchen docket.
...
events, availability, count and portion items, regulars, and the cook docket.
Routes include:

`/auth/login`, `/auth/register`, `/onboarding`, `/households/add`, `/home`,
`/events/new`, `/events/:id`, `/regulars`, and `/kitchen`.

Features not yet implemented include event editing/cancellation, cooked-status
automation, pantry ledger UI, allergies editing, push notifications, mobile
Tauri targets, and production Supabase configuration.
